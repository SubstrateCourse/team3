...
impl<T: Trait> Module<T> {
    ...
    fn insert_kitty(owner: T::AccountId, kitty_id: u32, kitty: Kitty) -> DispatchResult {
        ...
        let new_count = Self::kitties_count().checked_add(1).ok_or("error add")?;
        ...
        let user_kitties_count = user_kitties_id.checked_add(1).ok_or("error add")?;
        <OwnedKittiesCount<T>>::insert(owner, user_kitties_count);
