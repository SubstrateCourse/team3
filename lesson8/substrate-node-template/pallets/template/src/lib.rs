...
impl <T: Trait> Module<T> {
	fn submit_number(block_number: T::BlockNumber){
		...
		let lastst = if index > 0 {
			Self::numbers((index - 1) as u64)
		}else {
			0
		};

		let new: u64 = lastst.saturating_add((index + 1).saturating_pow(2));
		...
		for (_acc, res) in &results {
			match res {
				Ok(()) => {debug::native::info!("offchain tx succeeded: number: {}", new);}
				Err(_e) => {debug::error!("offchain tx failed: number: {}", new);}
			}
		}
