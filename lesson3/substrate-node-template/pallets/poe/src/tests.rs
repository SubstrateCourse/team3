...
#[test]
fn create_claim_works_failed_when_already_exists(){
    new_test_ext().execute_with(||{
        let claim = vec![0,1];
...
fn transfer_claim_success(){
    new_test_ext().execute_with(||{
        let claim = vec![0,1];
        let _ = PoeModule::create_claim(Origin::signed(1),claim.clone());
        assert_ok!(PoeModule::transfer_claim(Origin::signed(1),claim.clone(), 666));
    })
}
...
fn transfer_claim_failed_when_not_owner(){
    new_test_ext().execute_with(||{
        ...
        assert_noop!(
            PoeModule::transfer_claim(Origin::signed(2),claim.clone(), 666),
            Error::<Test>::NotClaimOwner
        );
    })
}
...
fn transfer_claim_failed_when_claim_not_exist(){
    new_test_ext().execute_with(||{
        ...
        // let _ = PoeModule::create_claim(Origin::signed(1),claim.clone());
        assert_noop!(
            PoeModule::transfer_claim(Origin::signed(1),claim.clone(), 666),
            Error::<Test>::ClaimNotExist
        );
    })
}